...
class Seccion_Model extends Models {
    ...
    function consultaZonasOpcionB($nivel) {
        ...
        if ($zonasActualizas != null) {
            ...
            for ($iZ = 0; $iZ < $zonas; $iZ++) {
                ...
                $arraySalida.="<h2>Sección: " . $nivel . "-" . $contadorSecciones . "</h2>";
    ...
    /* Carga la cantidad de estudiantes por especialidad */

    public function consultaEstadisticaEspecialidad($nivel) {
        $datos = array();
        $especialidades = $this->db->select("SELECT DISTINCT ee.cod_especialidad, e.nombreEspecialidad "
                . "FROM sipce_grupos as g,sipce_especialidad_estudiante as ee,sipce_especialidad as e  "
                . "WHERE ee.ced_estudiante = g.ced_estudiante "
                . "AND ee.cod_especialidad = e.codigoEspecialidad "
                . "AND g.nivel = " . $nivel . " "
                . "AND g.annio = " . ($this->anioActivo) . " "
                . "ORDER BY ee.cod_especialidad ");

        if ($especialidades != NULL) {
            $i = 0;
            foreach ($especialidades as $key => $value) {
                $datos[$i]['cod_especialidad'] = $value['cod_especialidad'];
                $datos[$i]['nombre_especialidad'] = $value['nombreEspecialidad'];

                $consul = $this->db->select("SELECT COUNT(ee.ced_estudiante) AS cantidadEstudiantes "
                        . "FROM sipce_especialidad_estudiante as ee, sipce_grupos as g "
                        . "WHERE ee.ced_estudiante = g.ced_estudiante "
                        . "AND g.nivel = " . $nivel . " "
                        . "AND g.annio = " . ($this->anioActivo) . " "
                        . "AND ee.cod_especialidad = " . $value['cod_especialidad'] . " ");
                foreach ($consul as $key => $value) {
                    $datos[$i]['cantidadEstudiantes'] = $value['cantidadEstudiantes'];
                }
                $i++;
            }
        }
    ...
    //Guardo la Cantidad de Secciones seleccionado en el nivel elegido//
    function guardarCantidadSeccionesDiversificada($consulta) {
        //Primero consulto si el nivel ya existe
        $consultaExistenciaZona = $this->db->select("SELECT * "
                . "FROM sipce_zona_secciones_diversificada "
                . "WHERE nivel = " . $consulta['nivel']);

        if ($consultaExistenciaZona != null) {
            //Actualizo datos del nivel
            $posData = array(
                'cantidadSecciones' => $consulta['cantidadSecciones']
            );
            $this->db->update('sipce_zona_secciones_diversificada', $posData, "`nivel` = {$consulta['nivel']}");
        } else {
            //Sino Inserto la Cantidad de Secciones y el nivel
            $this->db->insert('sipce_zona_secciones_diversificada', array(
                'nivel' => $consulta['nivel'],
                'cantidadSecciones' => $consulta['cantidadSecciones']));
        }
        //Luego devuelvo la Cantidad de Secciones Actualiza
        $cantidadSeccionesActualizas = $this->db->select("SELECT cantidadSecciones "
                . "FROM sipce_zona_secciones_diversificada "
                . "WHERE nivel = " . $consulta['nivel'] . " ");
        echo json_encode($cantidadSeccionesActualizas);
    }

    /* Carga la cantidad de secciones guardadas en la BD */

    public function consultaSeccionesDiversificada($nivel) {
        return $this->db->select("SELECT cantidadSecciones "
                        . "FROM sipce_zona_secciones_diversificada "
                        . "WHERE nivel = " . $nivel . " ");
    }

    /* Carga todas las especialidades guardadas en la BD */

    public function consultaEspecialidadNivel($nivel) {
        return $this->db->select("SELECT e.nombreEspecialidad "
                        . "FROM sipce_zona_secciones_especialidades as se, sipce_especialidad as e"
                        . "WHERE se.codigoEspecialidad = e.codigoEspecialidad "
                        . "AND se.nivel = " . $nivel);
    }

    //Carga las especialidades de la sección guardades en la BD//
    function consultaEspecialidadesSeccion($consulta) {
        $distritosZonaActualiza = $this->db->select("SELECT e.nombreEspecialidad, e.codigoEspecialidad "
                . "FROM sipce_especialidad as e, sipce_zona_secciones_especialidades as se "
                . "WHERE se.nivel = " . $consulta['nivelSeleccionado'] . " "
                . "AND se.grupo = " . $consulta['grupoSeleccionado'] . " "
                . "AND se.codigoEspecialidad = e.codigoEspecialidad "
                . "ORDER BY e.nombreEspecialidad");
        echo json_encode($distritosZonaActualiza);
    }

    //Guardo la especialidad seleccionado en la Seccion elegida//
    function agregarEspecialidad($consulta) {
        //Primero Inserto la especialidad
        $this->db->insert('sipce_zona_secciones_especialidades', array(
            'codigoEspecialidad' => $consulta['especialidad'],
            'nivel' => $consulta['nivelSeleccionado'],
            'grupo' => $consulta['grupoSeleccionado']));

        //Luego devuelvo la seccion Actualiza
        $especialidadesSeccionActualiza = $this->db->select("SELECT e.nombreEspecialidad, e.codigoEspecialidad "
                . "FROM sipce_especialidad as e, sipce_zona_secciones_especialidades as se "
                . "WHERE se.nivel = " . $consulta['nivelSeleccionado'] . " "
                . "AND se.grupo = " . $consulta['grupoSeleccionado'] . " "
                . "AND se.codigoEspecialidad = e.codigoEspecialidad "
                . "ORDER BY e.nombreEspecialidad");
        echo json_encode($especialidadesSeccionActualiza);
    }

    //Elimina la especialidad seleccionado//
    function eliminarEspecialidad($consulta) {
        //Primero Elimino la especialidad
        $this->db->delete('sipce_zona_secciones_especialidades', 'codigoEspecialidad = ' . $consulta['especialidad'] . ' AND nivel = ' . $consulta['nivelSeleccionado'] . ' AND grupo = ' . $consulta['grupoSeleccionado']);

        //Luego devuelvo la seccion Actualizada     
        $especialidadesSeccionActualiza = $this->db->select("SELECT e.nombreEspecialidad, e.codigoEspecialidad "
                . "FROM sipce_especialidad as e, sipce_zona_secciones_especialidades as se "
                . "WHERE se.nivel = " . $consulta['nivelSeleccionado'] . " "
                . "AND se.grupo = " . $consulta['grupoSeleccionado'] . " "
    ...
    //proyecta las secciones de educación diversificada//
    function proyectarSeccionesDiversificada($nivel) {
        $consultaSecciones = $this->db->select("SELECT DISTINCT grupo "
                . "FROM sipce_zona_secciones_especialidades "
                . "WHERE nivel = " . $nivel . " "
                . "ORDER BY grupo ");

        //este foreach guarda en un array las especialidades de cada sección [sección] [Especialidad]
        $arrayEspecialidades = array();
        foreach ($consultaSecciones as $key => $value) {
            $consultaEspecialidades = $this->db->select("SELECT codigoEspecialidad "
                    . "FROM sipce_zona_secciones_especialidades "
                    . "WHERE nivel = " . $nivel . " "
                    . "AND grupo = " . $value['grupo'] . " "
                    . "ORDER BY codigoEspecialidad ");
            $arrayEspecialidades [$value['grupo']] = $consultaEspecialidades;
        }

        //este foreach guarda en un array las especialidades de cada sección [sección] [Especialidad]
        $arrayEstudiantes = array();
        $grupo = 1;
        foreach ($arrayEspecialidades as $key => $value) {
            $subGrupo = 1;
            $codEspecialidad = 0;
            foreach ($arrayEspecialidades[$grupo] as $key => $value) {
                $resultado = $this->db->select("SELECT p.cedula,p.nombre,p.apellido1,p.apellido2,p.sexo,m.condicion,e.nombreEspecialidad "
                        . "FROM sipce_grupos as g,sipce_estudiante as p,sipce_especialidad_estudiante as ee,sipce_especialidad as e,sipce_matricularatificacion as m  "
                        . "WHERE g.annio = " . $this->anioActivo . " "
                        . "AND g.nivel = " . $nivel . " "
                        . "AND g.ced_estudiante = p.cedula "
                        . "AND g.ced_estudiante = ee.ced_estudiante "
                        . "AND p.cedula = m.ced_estudiante "
                        . "AND m.anio = " . $this->anioActivo . " "
                        . "AND e.codigoEspecialidad = ee.cod_especialidad "
                        . "AND ee.cod_especialidad = " . $arrayEspecialidades[$grupo][$codEspecialidad]['codigoEspecialidad'] . " "
                        . "ORDER BY p.apellido1,p.apellido2,p.nombre ");
                $arrayEstudiantes [$grupo][$subGrupo] = $resultado;
                $codEspecialidad++;
                $subGrupo++;
            }
            $grupo++;
